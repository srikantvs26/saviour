...
//		 when working with integers we should not use default value of 0, 1 etc because if
		// the list is empty then that default value don't give correct perception.
		...
		Optional<Integer> maxGrade = StudentDataBase.getAllStudents()
												.stream()
		...
		Optional<Integer> minGrade = StudentDataBase.getAllStudents()
												.stream()
												.map(student->student.getGradeLevel())
...
//		Arrays.asList(1,2,3,5,6).stream(); --> we can find min max the same for this as well.
